...
<!-- 内容区域 -->
<scroll-view scroll-y class="content-area" refresher-enabled @refresherrefresh="onRefresh" :refresher-triggered="isRefreshing">
<!-- 轮播图 -->
...
<uni-icons :type="getCreditIcon(item.creditLevel)" size="14" :color="getCreditColor(item.creditLevel)"></uni-icons>
<text :style="{ color: getCreditColor(item.creditLevel) }">{{ item.creditLevel }}</text>
</view>
</view>
...
.task-time text, .task-location text {
margin-left: 6rpx;
}
.task-footer {
display: flex;
